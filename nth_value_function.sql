...
FROM ORDERS
WHERE DATE_TRUNC(ORDER_DATE, MONTH) IN ('2024-01-01', '2024-01-02', '2024-01-03'); -- Replace with the desired months
...
-- find the first value of orders for a customer from the orders table over any given number of months based on chronological order (e.g., the latest date or month)
SELECT
    CUSTOMER_CODE,
    FIRST_VALUE(ORDERS) OVER (
        PARTITION BY CUSTOMER_CODE ORDER BY ORDER_DATE ASC -- Or another column representing chronological order
          ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
    ) AS FIRST_ORDER
FROM ORDERS
WHERE WHERE DATE_TRUNC(ORDER_DATE, MONTH) IN ('2024-01-01', '2024-01-02', '2024-01-03'); -- Replace with the desired months
